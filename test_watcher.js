...
var _phpunit_path = path.join(_dir, 'vendor/bin/phpunit');
...
check_phpunit_phar(function () {
    ...
    watch.createMonitor(
      ...
      , {
        ...
        , ignoreDotFiles: true
        , ignoreDirectoryPattern: /(node_modules|vendor|scripts|tools)/
        , filter: function (f, stat) {
            return stat.isDirectory()
                ? ['cache'].indexOf(path.basename(f)) < 0
                : path.extname(f) === '.php'
            ;
        }
      }
...
function run_test() {
    ...
    else if ( fs.existsSync(phpunit_xml = path.join(_test_dir, 'phpunit.xml.dist')) ) {
        args.push('-c');
        args.push(phpunit_xml);
    }
